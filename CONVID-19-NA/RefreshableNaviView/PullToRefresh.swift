...
public struct RefreshableList<Content: View>: View {
    ...
    public var body: some View {
        ...
        .onPreferenceChange(NavRefreshableKeyTypes.PrefKey.self) { values in
    ...
    var body: some View {
        GeometryReader{ geometry in
            RefreshView(isRefreshing: self.$showRefreshView, status: self.$pullStatus)
                .opacity(Double((geometry.frame(in: CoordinateSpace.global).origin.y - 106) / 80)).preference(key: NavRefreshableKeyTypes.PrefKey.self, value: [NavRefreshableKeyTypes.PrefData(bounds: geometry.frame(in: CoordinateSpace.global))])
                .offset(x: 0, y: -90)
        }
    }
}
